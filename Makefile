...
CFLAGS+=-O2 -Wall -Wno-write-strings -Wimplicit-function-declaration -D_LINUX
CPPFLAGS+=-std=c++11 
...
test: #$(TSTOOLLIB)
	$(CC) $(CFLAGS) $(APP_SRC) $(LIBS_RELEASE) $(LIBS) $(LDFLAG) -o test_change_$(PLATFORM)
	@echo === Target Platform [$(PLATFORM)] test change Done ===
	
clean:
ifeq ($(LIB),)
	rm -rf *.o *~ *.bak test_change_$(PLATFORM)*
else
